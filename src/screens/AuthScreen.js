...
export default class Auth extends React.Component {
  ...
  render() {
    ...
    const tabs = () => {
      const displayTab =
        tabIndex === 0 ? (
          <Login submit={this.performLogin} {...loginProps} />
        ) : (
          <Signup submit={this.performSignup} />
        );
      ...
      return (
        ...
        >
          <View style={styles.tabHeader}>
            <TouchableOpacity
              style={loginTabStyle}
              onPress={() => this.switchTabs(0)}
            >
              <Text style={loginTabTextStyle}>LOG IN</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={signupTabStyle}
              onPress={() => this.switchTabs(1)}
            >
              <Text style={signupTabTextStyle}>SIGN UP</Text>
            </TouchableOpacity>
          </View>
          <View style={styles.tabContent}>{displayTab}</View>
        </View>
      );
    };
